 ...
 */
@Service
public class CategoryServiceImpl extends ServiceImpl<CategoryMapper, BookCategory> implements ICategoryService {
    ...
    public void signalAdd(CategoryDTO categoryDTO) {
        ...
        int num = categoryMapper.insert(bookCategory);
        if(num != 1){
    ...
    public CategoryDTO getDetail(Long id) {
        ...
        BookCategory bookCategory = categoryMapper.selectById(id);
    ...
    public void deleteCategory(Long id) {
        ...
        int num = categoryMapper.deleteById(id);
        if(num != 1){
    ...
    public void batchDeleteCategory(List<Long> ids) {
        ...
        Long count = bookMapper.selectCount(new QueryWrapper<Book>().in("category_id", ids));
        if (count > 0) {
            throw new BaseException("部分分类已被使用，无法删除！");
        }
        int num = categoryMapper.deleteByIds(ids);
        if(num != ids.size()){
