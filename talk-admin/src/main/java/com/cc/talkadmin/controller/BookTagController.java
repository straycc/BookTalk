...
public class BookTagController {
    ...
    @ApiOperation("标签分页查询")
    @PostMapping("/page")
    public Result<PageResult<BookTagVO>> page(@RequestBody PageTagDTO pageTagDTO) {
         PageResult<BookTagVO> pageResult = bookTagService.getPage(pageTagDTO);
    ...
    public Result<Void> tagSignalAdd(@RequestBody BookTagDTO bookTagDTO) {
        bookTagService.tagSignalAdd(bookTagDTO);
    ...
    public Result<TagUpResult> tagsBatchAdd(@RequestBody List<BookTagDTO> bookTagsList) {
        TagUpResult tagUpResult =  bookTagService.tagsBatchAdd(bookTagsList);
    ...
    public Result<BookTagVO> getTagDetail(@PathVariable Long id) {
        BookTagVO bookTagVO = bookTagService.getDetail(id);
        return Result.success(bookTagVO);
    }
    ...
    public Result<Void> tagRevise(@PathVariable Long id, @RequestBody BookTagDTO bookTagDTO){
        bookTagService.tagRevise(id,bookTagDTO);
        return Result.success();
    }
    ...
    public Result<Void> tagSignalDelete(@PathVariable Long id) {
        bookTagService.tagSignalDelete(id);
        return Result.success();
    }
    ...
    public Result<Void> tagsBatchDelete(@RequestBody List<Long> idList) {
        bookTagService.tagsBatchDelete(idList);
        return Result.success();
    }
    ...
    @GetMapping("/list")
    public Result<List<BookTagVO>> getAllTags() {
        List<BookTagVO> bookTagVOList = bookTagService.list().stream().map(
                bookTag -> {
                    return ConvertUtils.convert(bookTag, BookTagVO.class);
                }
        ).collect(Collectors.toList());
        return Result.success(bookTagVOList);
    }
    ...
    public Result<List<BookTagVO>> listTagsByCategory(@PathVariable Long categoryId) {
        return Result.success(bookTagService.getByCategoryId(categoryId));
    }
    ...
    public Result<Boolean> checkTagExist(@RequestParam String name) {
        return Result.success(bookTagService.checkTagExist(name));
    }
