...
public class UserBehaviorInterestService {
    ...
    public void updateBookInterest(Long userId, Long bookId, Double behaviorScore) {
        ...
        // 更新具体标签兴趣
        for (String tag : tags) {
            userInterestService.updateInterestScore(userId, InterestType.TAG, tag, behaviorScore);
        }

        // 更新分类兴趣
        if (categoryId != null) {
            userInterestService.updateInterestScore(userId, InterestType.CATEGORY, String.valueOf(categoryId), behaviorScore);
        }

        // 更新作者兴趣
        if (book.getAuthor() != null && !book.getAuthor().trim().isEmpty()) {
            userInterestService.updateInterestScore(userId, InterestType.AUTHOR, book.getAuthor().trim(), behaviorScore);
        }
